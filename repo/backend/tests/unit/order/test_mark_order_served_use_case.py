...
def test_mark_order_served_happy_path() -> None:
    repository = FakeOrderRepository(_order(OrderStatus.READY))
    publisher = FakePublisher()

    response = MarkOrderServed(order_repository=repository, publisher=publisher).execute(
        order_id=OrderId("ord_001"),
...
def test_mark_order_served_rejects_non_ready_order() -> None:
    repository = FakeOrderRepository(_order(OrderStatus.ACCEPTED))

    with pytest.raises(InvalidOrderTransitionError):
        MarkOrderServed(order_repository=repository, publisher=FakePublisher()).execute(
            order_id=OrderId("ord_001"),
            trace_ctx=TraceContext(trace_id=None, request_id=None),
        )
