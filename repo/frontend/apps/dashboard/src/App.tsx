...
function App() {
  ...
  async function loadKitchenBoard(): Promise<void> {
    setLoadingQueue(true);
    setQueueError(null);
    try {
      const payloads = await Promise.all(
        kitchenStatuses.map(async (status) => {
          const response = await fetch(`${kitchenQueueEndpoint}?status=${status}&limit=50`);
          if (!response.ok) {
            throw new Error(formatApiError(await readApiError(response)));
          }
          return (await response.json()) as KitchenQueueResponse;
        })
      );
      mergeOrders(payloads.flatMap((payload) => payload.orders));
    } catch (error) {
      setQueueError(error instanceof Error ? error.message : "failed to load kitchen board");
    } finally {
  ...
  const kitchenBoard = useMemo(
    () =>
      kitchenStatuses.reduce<Record<StatusTab, OrderPayload[]>>(
        (current, status) => {
          current[status] = Object.values(orders)
  ...
  useEffect(() => {
    void loadKitchenBoard();
  }, [kitchenQueueEndpoint]);
      ...
      {view === "KITCHEN" ? (
        <section className="card">
          ...
          <div className="cardBody">
            {loadingQueue ? <div className="infoBox">Loading kitchen board…</div> : null}
            ...
            <div className="boardGrid">
              {kitchenStatuses.map((status) => (
                <section
                  key={status}
                  className={`column column${status.charAt(0)}${status.slice(1).toLowerCase()}`}
                >
                  <div className="columnHeader">
                    <div>
                      <h3 className="subheading">{status}</h3>
                      <p className="hint">Orders currently marked {status.toLowerCase()}.</p>
                    </div>
                    <span className={getOrderStatusChipClass(status)}>{kitchenBoard[status].length}</span>
                  </div>

                  {kitchenBoard[status].length === 0 && !loadingQueue ? (
                    <div className="emptyState">No orders in {status.toLowerCase()}.</div>
                  ) : null}

                  <div className="cardList">
                    {kitchenBoard[status].map((order) => {
                      ...
                      return (
                        <article key={order.orderId} className="orderCard">
                          ...
                          {order.lines.length > 0 ? (
                            ...
                            <p className="rowBody">No line items available.</p>
                          )}
